...
export default function Chat() {
    ...
    const handleOnChange = async(e: React.FormEvent<HTMLInputElement>) => {
        ...
        setFile(target.files[0])
            ...
            console.error("Error in handleOnChange");
        } 
  ...
  return (
    <>
        <Navbar></Navbar>
        <div className={clsx({"h-screen box-border  p-10":true, 
                            "hidden":isFileProcessed})}>
            ...
            >
                ...
                >
                 ...
                 <button type="submit" className="shrink-0 px-8 py-4 bg-sky-600 rounded w-28">
                    <div role="status" className={`${(isFileUploading) ? "" : "hidden"} flex justify-center`}>
                    ...
                    <span className={(isFileUploading) ? "hidden" : ""}>Upload</span>
                </button>
                </div>
            </form>
        
        </div>
        <div className={clsx({"hidden":!isFileProcessed})}> 
            <ChatWindow
            
            endpoint='api/documentqa'
            emoji="🤖"
            titleText="Mastishk"
